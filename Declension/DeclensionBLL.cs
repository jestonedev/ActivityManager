...
namespace BLL.Declension
{
    ...
    /// </remarks>
    public static class DeclensionBLL
    {
        ...
        private static readonly Encoding encoding = Encoding.GetEncoding("Unicode");
        ...
        {
            byte[] bRes = new byte[resultLen*2];
            Marshal.Copy(ptr, bRes, 0, resultLen*2);
        ...
        private static IntPtr StringToIntPtr(string srcStr)
        {
            ...
            /*IntPtr stringIn = Marshal.StringToHGlobalUni(srcStr);
            return stringIn;*/
            byte[] buf = new byte[srcStr.Length*2 + 2];
            encoding.GetBytes(srcStr, 0, srcStr.Length, buf, 0);
            buf[srcStr.Length * 2] = 0;
            buf[srcStr.Length * 2 + 1] = 0;
